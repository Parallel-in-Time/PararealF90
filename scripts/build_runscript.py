    ...
    with open("submit_"+type+"_Np"+str(Nproc)+".sh", "w") as myfile:
        ...
        if system=="cub":
            if type=="mpi":
                ...
                myfile.write('OMP_NUM_THREADS="+str(Nproc)+" mpirun -n 1 bin/parareal_openmp.out \n')
            elif type=="openmp_pipe":
                myfile.write('OMP_NUM_THREADS="+str(Nproc)+" mpirun -n 1 bin/parareal_openmp_pipe.out \n')
